...
# ------------------------------------------------------------------------------
# Project: Python siril script to run Retinex low light enhancer via spandrel
...
try:
    ...
    modelpath = os.path.join(siril.get_siril_configdir(),"FiveK.pth")

    if os.path.isfile(modelpath) :
        print("Retinex model found : "+modelpath)
    else :
        ssl._create_default_https_context = ssl._create_stdlib_context
        urllib.request.urlretrieve("https://drive.usercontent.google.com/u/0/uc?id=1oxvPPfhbOwZURTFenWnFp3H3Lakkqw3t&export=download", modelpath)
        print("Retinex model downloaded : "+modelpath)
    ...
    siril.update_progress("Retinex model initialised",0.05)
    ...
    for i, tile in enumerate(tile_process(model, imgcv2resized, scale, tile_size, yield_extra_details=True)):
        ...
        siril.update_progress("Retinex enhancement ongoing",p)
    ...
    siril.update_progress("Image enhanced",1.0)
